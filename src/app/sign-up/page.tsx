...
export default function SignUpPage() {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      ...
      if (errorParam === "oauth_failed") {
        ...
        errorMessage = "Session error. Please sign up again.";
  ...
  // Reset OAuth loading when pathname changes away from sign-up
  useEffect(() => {
    if (pathname !== "/sign-up" && oauthLoading) {
      console.log("[SignUp] Pathname changed away from /sign-up, resetting OAuth loading");
      setOauthLoading(false);
  ...
  useEffect(() => {
    const resetOAuthLoading = () => {
      // Only reset if we're still on sign-up page and OAuth is loading
      // and we don't have a code in URL (which means we're in callback)
      if (pathname === "/sign-up" && oauthLoading) {
        const code = searchParams.get("code");
        if (!code) {
          console.log("[SignUp] User returned, resetting OAuth loading");
          setOauthLoading(false);
          oauthStartTimeRef.current = null;
          if (timeoutRef.current) {
            clearTimeout(timeoutRef.current);
  ...
  useEffect(() => {
    if (oauthLoading) {
      ...
      timeoutRef.current = setTimeout(() => {
        console.log("[SignUp] OAuth timeout (12s), resetting loading");
        setOauthLoading(false);
  ...
  async function handleGoogleOAuth() {
    ...
    try {
      ...
      console.error("[SignUp] Unexpected error during OAuth:", err);
