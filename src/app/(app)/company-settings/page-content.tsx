...
export function CompanySettingsContent() {
  ...
  const { company: userCompany, loading: userCompanyLoading, isAuthenticated, refresh: refreshUserCompany } = useUserCompany();
    ...
    async function load() {
      try {
        ...
        setBranding({
          ...
          primaryColor: data.brand_color || "#2B74FF",
        ...
        if (response.ok) {
          ...
          const mapped: TeamRole[] = Array.isArray(members)
            ? members.map((m: any) => ({
                id: m.id,
                name: (m.name || m.email?.split("@")[0]) ?? "",
                email: m.email ?? "",
                role: m.role ?? "viewer",
                canEditTemplates: m.can_edit_templates ?? false,
                canChangeBranding: m.can_change_branding ?? false,
                canShareInvestorLinks: m.can_share_investor_links ?? false,
                canAccessDashboard: m.can_access_dashboard !== undefined ? m.can_access_dashboard : true,
                canViewOverview: m.can_view_overview !== undefined ? m.can_view_overview : true,
              }))
            : [];
          setTeamRoles(mapped);
        }
