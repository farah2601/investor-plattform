 ...
 * Used by MCP agent tools to read directly from Google Sheets
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.getSheetsCsvUrl = getSheetsCsvUrl;
 ...
 */
function getSheetsCsvUrl(sheetUrl, tabName) {
 ...
 */
function parseMonthToPeriodDate(monthStr, year) {
    if (!monthStr)
        return null;
 ...
 */
async function fetchGoogleSheetsData(googleSheetsUrl, googleSheetsTab) {
            ...
            for (let rowIndex = 1; rowIndex < rows.length; rowIndex++) {
                ...
                for (const [colIndexStr, fieldName] of Object.entries(columnMap)) {
                    ...
                    if (parsedValue !== null) {
                        ...
                        if (fieldName === "mrr")
                            snapshotPayload.mrr = parsedValue;
                        else if (fieldName === "arr")
                            snapshotPayload.arr = parsedValue;
                        else if (fieldName === "burn_rate")
                            snapshotPayload.burn_rate = parsedValue;
                        else if (fieldName === "churn")
                            snapshotPayload.churn = parsedValue;
                        else if (fieldName === "growth_percent")
                            snapshotPayload.growth_percent = parsedValue;
                        else if (fieldName === "runway_months")
                            snapshotPayload.runway_months = parsedValue;
                        else if (fieldName === "lead_velocity")
                            snapshotPayload.lead_velocity = parsedValue;
                        else if (fieldName === "cash_balance")
                            snapshotPayload.cash_balance = parsedValue;
                        else if (fieldName === "customers")
                            snapshotPayload.customers = parsedValue;
                    }
                }
                // Only add snapshot if it has at least one KPI value
                const hasAnyValue = Object.values(snapshotPayload).some((val, idx) => idx > 0 && val !== null // Skip period_date (index 0)
                );
                if (hasAnyValue) {
